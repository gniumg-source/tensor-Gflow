...
absl::StatusOr<IndexingMapWithAdditions> GetNewIndexingMapAfterFoldingSequence(
  ...
  for (auto& [operand_id, producer] : apply_indexing_ops) {
    ...
    int num_producer_dims = producer_map.GetDimensionCount();
    ...
    for (auto& producer_operand : producer->getOpOperands()) {
      ...
      if (!replacement_expr) {
        ...
        // TODO(karupayun): CHECK THIS
        replacement_expr =
            CreateSymbolExpr(added_dim_args.size(), num_dims, ctx);
  ...
  LogicalResult matchAndRewrite(ApplyIndexingOp indexing_op,
    ...
    for (auto [operand, constant_value] :
      ...
      if (constant_value.has_value()) {
        ...
        dim_replacements.push_back(CreateSymbolicVariable(new_num_dims++, ctx));
    ...
    for (mlir::OpResult opresult : indexing_op->getOpResults()) {
      ...
      if (result_expr.GetType() == SymbolicExprType::kConstant) {
        new_values.push_back(arith::ConstantIndexOp::create(
            rewriter, loc, result_expr.Evaluate({})));
        continue;
      }
      if (IsDimension(result_expr, indexing_map.GetDimVarsCount())) {
        new_values.push_back(indexing_op.getOperand(
            GetDimensionIndex(result_expr, indexing_map.GetDimVarsCount())));
        continue;
      }
      if (IsSymbol(result_expr, indexing_map.GetDimVarsCount())) {
        new_values.push_back(indexing_op.getOperand(
            GetSymbolIndex(result_expr, indexing_map.GetDimVarsCount())));
        continue;
      }
